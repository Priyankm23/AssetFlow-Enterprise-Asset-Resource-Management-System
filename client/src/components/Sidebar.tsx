...
export function Sidebar({
  ...
  const visibleNavItems = navItems
    ...
    .filter(
      (item, idx, arr) => arr.findIndex((x) => x.route === item.route) === idx,
    );
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 h-14 border-b border-ink-700/50">
          ...
          <button
            onClick={() => setMobileOpen(false)}
            className="lg:hidden text-ink-300"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <nav className="flex-1 py-3 px-2.5 overflow-y-auto scrollbar-thin">
          {visibleNavItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-4 h-4 shrink-0 ${active ? "text-accent-400" : "text-ink-300"}`}
                />
        ...
        <div className="px-3 py-3 border-t border-ink-700/50">
          <div className="flex items-center gap-2.5 px-2 py-2 rounded-lg">
            ...
            <button
              onClick={logout}
              className="text-ink-300 hover:text-status-lost transition-colors p-1"
              title="Sign out"
            >
